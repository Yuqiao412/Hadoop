...
import com.example.hadoopdemo.mapmatching.mapreduce.congestion.CongestionReducer2ByHour;
...
@Component
public class FlowCountTotalJob extends AbstractJob {

	private final Configuration config;
	private final HdfsUtils hdfsUtils;
	String date = "20171204";

	public FlowCountTotalJob(Configuration config, HdfsUtils hdfsUtils) throws IOException {
		this.config = config;
	...
	public JobStatus execute() throws IOException, ClassNotFoundException, InterruptedException {
		...
		hdfsUtils.removeDir("/flowcount/"+date);
		...
		FileOutputFormat.setOutputPath(job, new Path("hdfs://192.168.1.11:9000/flowcount/"+date));
